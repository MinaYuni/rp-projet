...
def consistance_locale(instanciation_partielle, contraintes):
    # TODO
    return True


def consistance_globale(instanciation, contraintes, dictionnaire):
...
def instancier_variable(lettres_restantes, instanciation_partielle):
    """
    Instancie la variable var en vérifiant les contraintes locales.
    """
    ...
    while lettres_restantes:
        ...
        if consistance_locale(instanciation_courante, []):  # TODO
            return True, lettres_restantes, instanciation_courante

    return False, [], instanciation_partielle


def reduire_domaine(lettres_a_retirer, domaine):
    """
    Réduit un domaine en retirant certaines lettres. Changement "in place".
    """

    for l in lettres_a_retirer:
        domaine.remove(l)
